...
export function bootstrap(container: HTMLDivElement | null): void {
  ...
  const layout = createLayout({
    ...
    onInputApply: (config) => {
      ...
      // Generate updated code with new values
  ...
  function generateUpdatedCode(
    ...
    let searchVarName: string | null = null;
    if (selectedLesson.watchedVariables.includes('target')) {
      searchVarName = 'target';
    }
    ...
    for (const line of lines) {
      ...
      if (/^(while|for|if|function)\b/.test(trimmed)) {
        updatedLines.push(line);
        continue;
      }

      // Replace arr assignment
      if (trimmed.startsWith('arr') && trimmed.includes('=') && !foundArr) {
        const arrayStr = config.arrayValues.join(', ');
        updatedLines.push(`arr = [${arrayStr}];`);
        foundArr = true;
        continue;
      }

      // Replace search variable assignment (dynamic variable name)
      if (
        searchVarName &&
        trimmed.startsWith(searchVarName) &&
        trimmed.includes('=') &&
        !foundSearchVar &&
        config.target !== null
      ) {
        updatedLines.push(`${searchVarName} = ${config.target};`);
        foundSearchVar = true;
        continue;
      }
  ...
  function calculateExpectedOutput(lesson: LessonDefinition): string {
    ...
    // Parse array values
    const arrayValuesText = layout.inputControl.arrayValuesInput.value;
    const arrayValues = arrayValuesText
      .split(',')
      .map((s) => parseInt(s.trim(), 10))
      .filter((n) => !isNaN(n));

    // If no values, show placeholder
    if (arrayValues.length === 0) {
      return 'Configure your input values above to see the expected output.';
    }

    const algorithmType = lesson.algorithmType;

    // Search algorithms
    if (algorithmType === 'search') {
      const found = arrayValues.includes(targetValue || -1);
      const foundIndex = arrayValues.indexOf(targetValue || -1);
      const searchVarName = lesson.watchedVariables.includes('target') ? 'target' : null;
      if (searchVarName) {
        return `Array: [${arrayValues.join(', ')}]\n${searchVarName}: ${targetValue}\n\nExpected Output: answer = ${found ? foundIndex : -1}${found ? ' (found)' : ' (not found)'}`;
      }
    }

    // Sorting algorithms
    if (algorithmType === 'sort') {
      const sorted = [...arrayValues].sort((a, b) => a - b);
      return `Array: [${arrayValues.join(', ')}]\n\nExpected Output: [${sorted.join(', ')}]`;
    }

    // Scanning algorithms
    if (algorithmType === 'scan') {
      if (lesson.id.includes('sum')) {
        const sum = arrayValues.reduce((a, b) => a + b, 0);
        return `Array: [${arrayValues.join(', ')}]\n\nExpected Output: sum = ${sum}`;
      }
      if (lesson.id.includes('max')) {
        const max = Math.max(...arrayValues);
        return `Array: [${arrayValues.join(', ')}]\n\nExpected Output: max = ${max}`;
      }
      if (lesson.id.includes('min')) {
        const min = Math.min(...arrayValues);
        return `Array: [${arrayValues.join(', ')}]\n\nExpected Output: min = ${min}`;
      }
      if (lesson.id.includes('average')) {
        const sum = arrayValues.reduce((a, b) => a + b, 0);
        const avg = (sum / arrayValues.length).toFixed(2);
        return `Array: [${arrayValues.join(', ')}]\n\nExpected Output: average = ${avg}`;
      }
    }

    return lesson.expectedOutputDescription || 'Run the algorithm to see the output.';
  }
  ...
  function render(): void {
    ...
    // Render explanation with boxes
    const descriptionContent = layout.explanation.descriptionBox.querySelector('.box-content');
    if (descriptionContent) {
      descriptionContent.replaceChildren(paragraph(selectedLesson.fullDescription || selectedLesson.description));
    }

    // Update example box in explanation panel
    const exampleContent = layout.explanation.exampleBox?.querySelector('.box-content');
    if (exampleContent) {
      exampleContent.replaceChildren(paragraph(selectedLesson.expectedOutputDescription || 'No example provided.'));
    }

    // Calculate and display dynamic expected output in input panel
    const expectedOutput = calculateExpectedOutput(selectedLesson);
    const currentOutputContent = layout.inputControl.currentOutputBox?.querySelector('.output-box-content');
    if (currentOutputContent) {
      const formattedInputOutput = expectedOutput.split('\n').map(line => {
        const p = document.createElement('p');
        p.textContent = line;
        p.style.whiteSpace = 'pre-wrap';
        p.style.fontFamily = "'Cascadia Code', 'Consolas', monospace";
        return p;
      });
      currentOutputContent.replaceChildren(...formattedInputOutput);
    }
